# Research dossier — Jeff Dean
# (broad research; factual coverage=0.75, gap-filled 20, 39 live-reasoning threads excluded [deep-research backend])
...
Here is the supplement covering the missing facts, based on the transcript provided.

**Education & Early Career**
- Jeff Dean's undergraduate honors thesis focused on implementing and comparing parallelization methods (model and data parallelism) for neural network backpropagation training on a 32-processor Hypercube machine in 1990.
- Before Google, he worked at the World Health Organization's Global Programme on AIDS (1990–1991).
- He also worked at Digital Equipment Corporation's Western Research Lab (1996–1999), where his work involved profiling tools and microprocessor design.

**Leadership Roles & Career Timeline**
- In 2018, Jeff Dean was named to lead Google AI.
- Following the April 2023 merger of Google Brain and DeepMind into Google DeepMind, he became Google's Chief Scientist.
- He has left and returned to Google more than once over his career.

**Honors & Awards**
- Jeff Dean was elected to the National Academy of Engineering in 2009 and the American Academy of Arts and Sciences in 2016.
- He is a fellow of the ACM and the AAAS.
- He shared the 2012 ACM Prize in Computing with his longtime collaborator, Sanjay Ghemawat.
- His other honors include the IEEE John von Neumann Medal, a NeurIPS 2023 Test of Time Award, and multiple SIGOPS Hall of Fame entries.
- In September 2023, *Time* magazine named him among the 100 most influential people in AI.

**Key Systems & Performance Metrics**
- As of 2023, Bigtable serves more than 6 billion requests per second at peak.
- Spanner manages over 10 exabytes of data and won a 2025 ACM SIGMOD Systems Award.

**Research & Technical Contributions**
- The central thesis of "The Tail at Scale" paper is that eliminating all variability is impractical, so systems should be tail-tolerant. It proposes using techniques like hedged requests, tied requests, and micro-partitioning, drawing an analogy to fault-tolerant computing.
- The canonical figures for "Numbers Everyone Should Know" trace back to Peter Norvig.
- The 2021 *Nature* paper on chip floorplanning later drew external disputes over reproducibility from some chip-design researchers.
- This chip-design work underpinned the PaLM model (540 billion parameters).
- "The Case for Learned Index Structures" reported that learned indexes could outperform cache-optimized B-trees by up to 70% in speed while offering an order-of-magnitude savings in memory.
- Skeptics outside Google contend that the per-prompt efficiency figures for such systems exclude training and embodied-hardware costs.

**Foundational AI Architectures & Collaborations**
- The central claim of the "Attention Is All You Need" paper—that attention alone, without recurrence, suffices for sequence transduction—became the architectural foundation for essentially all subsequent large language models.
- Noam Shazeer's startup, Character.AI, emphasized personalized, always-available AI companions, and Shazeer publicly tied the bet to progress toward AGI.
- Dean and Shazeer's broader research programs are complementary: Dean supplies the distributed-systems substrate, hardware (TPUs), and the sparsity-as-efficiency thesis (Pathways), while Shazeer supplies the architectural primitives (the Transformer, MoE routing, MQA, SwiGLU) that those systems run.
- PaLM and the Switch Transformer sit at the intersection of both lines of work.
- The Gemini model reflects both Dean's scaling/efficiency/Pathways agenda and Shazeer's Transformer-and-sparsity architecture lineage.
