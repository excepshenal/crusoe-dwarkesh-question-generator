...
# (broad research; factual coverage=0.467, gap-filled 48, 34 live-reasoning threads excluded [deep-research backend])
...
**Dylan Patel – Personal & Professional Background**
- Worked as a beekeeper in Minnesota for roughly 18 months after college.
- Runs SemiAnalysis, which operates a chip-teardown lab in Oregon called STEEL.

**SemiAnalysis – Business & Product Details**
- Paid newsletter tier reported at roughly $500/year.
- Advertises 14+ industry models and 8+ analytical tools, including an AI Cloud TCO Model, an AI Accelerator & HBM Model, a Datacenter Industry Model tracking 5,000+ datacenters, a Foundry model, GPU Pricing Index, and Inference Simulator.
- Reported headcount ranges from roughly 60 to 85 employees across multiple countries.
- Notable team members include president Doug O'Laughlin, and analysts/co-authors Afzal Ahmad, Kimbo Chen, Jeremie Eliahou Ontiveros, and Jordan Nanos.
- Reported newsletter reach varies by source and date — figures of 180,000+, 200,000+, and 290,000+ subscribers all appear.
- Per The Information, SemiAnalysis projected $100M+ revenue in 2026, up from roughly $20M the prior year.

**Key Client Engagements**
- Microsoft CEO Satya Nadella reportedly subscribed after a 2023 search-cost analysis.
- AMD CEO Lisa Su personally engaged after a December 2024 ROCm report.

**Notable SemiAnalysis Publications & Estimates**
- The 'Google: We Have No Moat' piece published a leaked internal Google document later attributed to engineer Luke Sernau.
- The 'GPT-4 Architecture' piece estimated ~1.8 trillion parameters, a mixture-of-experts design (16 experts of ~111B each, top-2 routing), ~13 trillion training tokens, ~25,000 A100s for 90–100 days, ~32–36% MFU, and a single-run hardware training cost of roughly $63M.
- Patel separately estimated GPT-4's all-in cost, including R&D and multiple runs, at closer to $500M.
- The 'Inference Cost Of Search Disruption' piece estimated that putting ChatGPT-style inference into all Google search would require ~4.1 million A100s, over $100B in capex, and a ~$36B operating-income hit.
- Supporting numbers for the supply constraint thesis: AI consuming ~60% of TSMC N3 output in 2026 rising to ~86% in 2027; HBM using 3–4x the wafer area of commodity DRAM; and EUV/ASML becoming the leading constraint by 2030.
- The 'GPU-rich vs GPU-poor' distinction was popularized in the August 2023 'Google Gemini Eats The World' piece.
- The economic logic that 'an AI cloud can generate $10–12 billion per gigawatt annually' justifies behind-the-meter gas.
- The 'How AI Labs Are Solving the Power Crisis' piece cited US AI power demand rising from ~3 GW in 2023 to 28 GW+ in 2026, with examples including the OpenAI/Oracle 2.3 GW Texas gas plant.
- The 'xAI's Colossus 2' piece targeted 1.1 GW with ~245 MW of onsite gas turbines.
- SemiAnalysis published that a DGX H100 retailed near $270,000 with ~$190,000 gross profit, and that H100 gross margin 'exceeds 85%', with HBM the single largest bill-of-materials component (~$1,150 of an estimated ~$3,000 BOM).
- The defining AMD dispute episode is 'MI300X vs H100 vs H200 Benchmark Part 1: Training' published December 22, 2024, after ~5 months of testing.
- SemiAnalysis found MI300X's superior paper specs failed to translate because AMD's ROCm software was crippled.
- Lisa Su personally called Patel (a 30-minute call ran ~90 minutes), AMD entered 'wartime mode', added MI300 to PyTorch CI/CD, and SemiAnalysis delayed publication to include a December dev build.
- The analysis was largely validated by Austin Lyons (Chipstrat).
- The 'Google AI Infrastructure Supremacy' piece noted Google used 48 optical-circuit-switches per 4,096 TPUs versus ~568 InfiniBand switches for a comparable cluster.
- The 'Google TPUv7' piece estimated TPUv7 Ironwood within ~10% of GB200 on peak specs with ~44% lower internal TCO, framing the ~1M-TPU Anthropic deal (~$49B).
- The 'Amazon's AI Self Sufficiency' piece detailed Trainium2 and Project Rainier (400,000 Trainium2 chips in Indiana), while ranking AWS a 'distant second' to Google in custom silicon.
- The 'Scaling the Memory Wall' piece updated mid-2025 HBM share to roughly SK Hynix 62% / Micron 21% / Samsung 17% and stated 'HBM failures are the #1 cause of GPU failures'.
- The 'Nvidia's Optical Boogeyman' piece argued fears that NVL72 copper would kill optics were overblown.
- The 'The New AI Networks' piece documented Ethernet clawing share from InfiniBand, noting Nvidia's own Spectrum-X Ethernet out-shipping its Quantum InfiniBand.

**Counterarguments & Critiques of Patel’s Thesis**
- Strongest counterarguments come from Jim Covello (Goldman Sachs), David Cahn (Sequoia), Daron Acemoglu (MIT), Gary Marcus, Epoch AI, and Tim De Chant (TechCrunch).
- CSIS's Gregory Allen called the DeepSeek cost estimate 'credible but sourced from anonymous industry contacts'.
- Nathan Lambert (Ai2) corroborated both halves of the DeepSeek cost debate.
- Patel was himself a critic of Alexandr Wang's '50,000 H100s' claim for lack of proof.
- Academic critics (Madhavi Venkatesan and Philip Hanser of Northeastern; an arXiv/FAccT paper) argued the Jevons paradox framing oversimplifies.
- Jensen Huang calls export controls a 'strategic failure'.
- ITIF and CSIS's Renewing American Innovation argue controls spurred Chinese self-sufficiency.
- The Register's Tobias Mann used SemiAnalysis's own power figures to argue Huawei is a 'boogeyman' not at real parity.
- Gregory Allen rebuts the 'controls have already failed' framing as premature.
- A June 2026 co-packaged-optics (CPO) 'delay' report triggered an optics sell-off (AAOI −17%) and was contradicted by Nvidia's Gilad Shainer and rebutted by Global Semi Research.
- Michael Burry and Jim Chanos argue a 2–3-year useful life implies a 'depreciation time bomb'; Patel calls the 2–3-year assumption a 'fatal flaw', citing 5-year-plus A100 reuse.
- Patel popularized the 'infinite money glitch' meme for the Nvidia-OpenAI-Oracle-CoreWeave financing loop but does not endorse the
